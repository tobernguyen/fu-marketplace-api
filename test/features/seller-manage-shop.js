...
describe('PUT /api/v1/seller/shops/:id', () => {
  ...
  before(done => {
    helper.factory.createUserWithRole({}, 'seller').then(u => {
      ...
      shop = s;
      return helper.factory.createShopWithShipPlace({banned: true, userId: owner.id}, 'dom A');
    }).then(s => {
      bannedShop = s;
      return helper.factory.createUser();
    }).then(u => {
...
describe('POST /api/v1/seller/shops/:id/uploadAvatar', () => {
  ...
  before(done => {
    helper.factory.createUserWithRole({}, 'seller').then(u => {
      ...
      shop = s;
      return helper.factory.createShopWithShipPlace({banned: true, userId: ownerId}, 'dom A');
    }).then(s => {
      bannedShop = s;
      done();
    });
